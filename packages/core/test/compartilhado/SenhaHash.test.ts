import SenhaHash from "../../src/compartilhado/SenhaHash"
...
test('Deve criar senha com hash válido', () => {
    ...
    expect(new SenhaHash(hashs[0] as string)).toBeDefined()
    expect(new SenhaHash(hashs[1] as string)).toBeDefined()
    expect(new SenhaHash(hashs[2] as string)).toBeDefined()
    expect(new SenhaHash(hashs[3] as string)).toBeDefined()
})
